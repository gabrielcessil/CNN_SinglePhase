...
def Bias_Comparison(batch_inputs, batch_outputs, batch_targets):
    if batch_outputs.shape[1] > 1:
        print("Bias Error: using velocity magnitude")
        ...
        print("Bias Error: using Single Component")
...
def Divergent_Residual(batch_inputs, batch_outputs):
    ...
    for s_i in range(batch_inputs.shape[0]):
        ...
        div     = (duz_dz + duy_dy + dux_dx).abs()
        abs_div = (duz_dz.abs() + duy_dy.abs() + dux_dx.abs())
        mask    = abs_div>0
        mean_i  = (100*div[mask]/abs_div[mask]).mean()
...
def Angular_Comparison(batch_inputs, batch_outputs, batch_targets):
    ...
    for s_i in range(mag_true.shape[0]):
        dot_i           = dot[s_i:s_i+1]
        den_i           = den[s_i:s_i+1]
        s_i_mask        = (batch_inputs[s_i:s_i+1]>0) & (den_i > 0)
        ...
        mean            = theta_deg.mean()
        std             = theta_deg.std()
        print(" -- Sample {}: mean={:.4f}º; std={:.4f}".format(s_i, mean.item(), std.item()))
        ae_means.append(mean.item())
    print(f"Mean: {np.mean(ae_means):.4f}º, Std: {np.std(ae_means):.4f}º")
    print("------------------------------------------------------------------")
    return ae_means
