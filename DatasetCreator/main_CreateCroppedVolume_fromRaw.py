...
for base_dir, rock_name in repositories:
    ...
    for i, sample_i in enumerate(indices):
        ...
        if include_walls: x_sample = utils.add_enclusure_walls(x_sample)
            
        if remove_isolated: filt_vol = utils.remove_isolated_pores(x_sample)
        else: filt_vol = x_sample
    ...
    utils.generate_slurm_run_scripts_chunks_GRADLBM(
        ...
        cpu_per_sim         = 1, 
        mem_gb_per_sim      = 6,
        dispatcher_name     = f"Run_GRAD_{0}_{created}.sh",
