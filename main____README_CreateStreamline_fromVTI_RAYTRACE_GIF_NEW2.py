...
class ParaViewRenderer:
    ...
    def _desaturate_and_lighten(rgb, desat_factor=0.5, lighten_offset=0.0):
        # Calculate standard relative luminance (pure gray value)
        gray = 0.299 * rgb[0] + 0.587 * rgb[1] + 0.114 * rgb[2]
        
        # Blend original color towards gray (1.0 = pure gray, 0.0 = original color)
        return [
    ...
    def _setup_view(self):
        ...
        if hasattr(self.view, 'EnvironmentalBG'):
            self.view.EnvironmentalBG = [0.6, 0.5803921568627451, 0.8]
    ...
    def _create_custom_wireframe(self, bounds):
        """
        Create the custom stepped wireframe from the actual dataset bounds.
        """
        ...
        wireframe_coords = [
            ...
            ([xc, yc, zc], [xc, ymax, zc]),
            ([xc, yc, zc], [xc, yc, zmax]),
            ([xmin, yc, zc], [xmin, ymax, zc]),
            ([xmin, yc, zc], [xmin, yc, zmax]),
            ([xc, ymax, zc], [xmin, ymax, zc]),
            ([xc, ymax, zc], [xc, ymax, zmax]),
            ([xc, yc, zmax], [xmin, yc, zmax]),
            ([xc, yc, zmax], [xc, ymax, zmax]),
            
            # 2. FULL OUTER DOMAIN LINES
            ...
            # 3. TRUNCATED OUTER DOMAIN LINES
            ([xmin, ymax, zmin], [xmin, ymax, zc]),
            ([xmax, ymax, zmax], [xc, ymax, zmax]),
            ([xmin, ymin, zmax], [xmin, yc, zmax])
        ]
    ...
    def render_file(self, file_path, output_filename, frame_idx, override_camera_pos, override_view_up):
        ...
        # 6. SOLID COLOR / OPACITY TRANSFER FUNCTIONS
        solid_lut = GetColorTransferFunction(self.config['scalar_name'])
        c_lut = self.config['solid_lut_color']
        solid_lut.RGBPoints = [
            -1e10, c_lut[0], c_lut[1], c_lut[2],
            0.0,   c_lut[0], c_lut[1], c_lut[2],
            1e10,  c_lut[0], c_lut[1], c_lut[2]
        ]
        ...
        # --- GLOW EFFECT FOR STREAMLINES ---
        disp_stream.Ambient = 1.0   # 100% self-illuminating
        disp_stream.Diffuse = 0.0   # 0% shadowed by environment
        disp_stream.Specular = 0.0  # Remove plastic-like shine
        disp_stream.OSPRayMaterial = "None"

        # ======================================================================
        # 8. STREAMLINE COLOR MAP
        # ======================================================================
        self.lut = GetColorTransferFunction(self.config['vector_name'])
        self.lut.ApplyPreset("Plasma (matplotlib)", True)
        
        # --- CLIP COLORMAP START ---
        # 1. Calculate actual velocity range (Moved from Step 9)
        ...
        # 2. Extract current color points [x, r, g, b, x, r, ...]
        pts = list(self.lut.RGBPoints)
        x_coords = pts[0::4]
        old_min, old_max = min(x_coords), max(x_coords)
        
        # 3. Discard the bottom 25% of the colormap (the dark purple start)
        clip_fraction = 0.1 
        cutoff = old_min + clip_fraction * (old_max - old_min)
        
        # 4. Rebuild the list with rescaled coordinates
        new_pts = []
        for i in range(0, len(pts), 4):
            x, r, g, b = pts[i:i+4]
            if x >= cutoff:
                # Normalize remaining x values to 0.0 - 1.0, then scale to velocity range
        ...
        # --- GLOW EFFECT FOR VOLUME ---
        disp_vol.Shade = 1
        disp_vol.Ambient = 0.8
        disp_vol.Diffuse = 0.8
        ...
        volume_min_opacity = 0.05 # 0.0 starts from transparent
        volume_max_opacity = 0.2 # 0.8 end on almost solid
        
        
        self.pwf.Points = [
            0.0, 0.0, 0.5, 0.0,
            max_vel*0.00001,                0.0, 0.5, 0.0,
            max_vel*0.0001,                 volume_min_opacity, 0.5, 0.0,
            max_vel*0.1,                    volume_max_opacity, 0.5, 0.0,
        ...
        # 11. HELPER FUNCTIONS FOR VISIBILITY
        def set_wireframe_visibility(value):
            for disp in self.wireframe_displays:
                disp.Visibility = value

        def set_solid_visibility(value):
            for disp in solid_displays:
                disp.Visibility = value
        ...
        # 12. PANEL 1 — SOLID ONLY
        set_solid_visibility(1)
        set_wireframe_visibility(1)
        disp_stream.Visibility = 0
        disp_vol.Visibility = 0
        disp_stream.SetScalarBarVisibility(self.view, False)

        SaveScreenshot(
            temp_1, self.view, ImageResolution=self.config['resolution'],
            TransparentBackground=0, OverrideColorPalette='WhiteBackground'
        )

        # ======================================================================
        # 13. PANEL 2 — SOLID + STREAMLINES
        # ======================================================================
        set_solid_visibility(1)
        set_wireframe_visibility(1)
        disp_stream.Visibility = 1
        disp_vol.Visibility = 0
        disp_stream.Opacity = 1.0
        
        # Desaturate and lighten based on user config variables
        desat_factor = self.config.get('panel2_desat_factor', 0.5)
        gray_diffuse = self._desaturate_and_lighten(
            self.config['solid_diffuse'],
            desat_factor=desat_factor,
            lighten_offset=self.config.get('panel2_lighten_diffuse', 0.15)
        )
        gray_ambient = self._desaturate_and_lighten(
            self.config['solid_ambient'],
            desat_factor=desat_factor,
            lighten_offset=self.config.get('panel2_lighten_ambient', 0.10)
        )

        for disp_solid in solid_displays:
            disp_solid.DiffuseColor = gray_diffuse
            disp_solid.AmbientColor = gray_ambient
            disp_solid.Opacity = 0.45
            disp_solid.Specular = 0.25
            disp_solid.SpecularPower = 30.0
            disp_solid.OSPRayMaterial = "None"
        
        disp_stream.SetScalarBarVisibility(self.view, False)
        
        SaveScreenshot(
            temp_2, self.view, ImageResolution=self.config['resolution'],
            TransparentBackground=0, OverrideColorPalette='WhiteBackground'
        )

        # 14. PANEL 3 — VOLUME + STREAMLINES
        set_solid_visibility(0)
        set_wireframe_visibility(0)
        disp_stream.Visibility = 1
        disp_vol.Visibility = 1
        disp_stream.Opacity = 0.5
        
        disp_stream.SetScalarBarVisibility(self.view, False)
        self.view.Shadows = 0
        
        SaveScreenshot(
            temp_3, self.view, ImageResolution=self.config['resolution'],
            TransparentBackground=0, OverrideColorPalette='WhiteBackground'
        )

        # 15. COMBINE PANELS WITH MATPLOTLIB
        fig, axes = plt.subplots(1, 3, figsize=(18, 6))
        fig.patch.set_facecolor("white")

        titles = [
            "Solid Only",
            "Streamlines in Crystal Solid",
            "Fluid Volume within Streamlines"
        ]
        ...
        plt.savefig(
            final_out, dpi=120, bbox_inches="tight", pad_inches=0.05,
            facecolor="white", edgecolor="white"
        )
        plt.close(fig)

        # 16. DELETE TEMPORARY SCREENSHOTS
        ...
        # 17. CLEANUP PARAVIEW OBJECTS
        for disp in self.wireframe_displays:
            try: Delete(disp)
            except: pass

        for disp in (solid_displays + [disp_stream, disp_vol]):
            try: Delete(disp)
            except: pass
        ...
        for item in [self.lut, self.pwf, solid_lut, solid_pwf, self.view]:
            try: Delete(item)
            except: pass
...
def path_static(config, total_frames):
    return (
        [config['camera_pos']] * total_frames,
        [config['camera_view_up']] * total_frames
    )
...
def generate_camera_trajectory(config, total_frames):
    ...
    if path_type == 'static':
        return path_static(config, total_frames)
    if path_type == 'circular':
        return path_circular(config, total_frames)
    raise ValueError("Unknown camera path type.")
...
config = {
    ...
    # SOLID COLORS
    # --------------------------------------------------------------------------
    'solid_ambient':  [0.6, 0.5804, 0.6706],#[0.6, 0.5804, 0.6706], #[0.45, 0.55, 0.45],
    'solid_diffuse':  [0.6, 0.5804, 0.6706],#[0.6, 0.5804, 0.8],  #[0.55, 0.65, 0.55],
    'solid_lut_color':[0.6, 0.5804, 0.6706],#[0.6, 0.5804, 0.8],  #[0.55, 0.62, 0.55],
    
    # PANEL 2 TRANSPARENT GRAY TINT SETTINGS
    # 0.0 = completely colored, 1.0 = purely gray. 
    'panel2_desat_factor': 0.2,
    'panel2_lighten_diffuse': 0.15, # brightness to the illuminated parts of the solid
    'panel2_lighten_ambient': 0.05, # brightness to the shadows of the solid
    # --------------------------------------------------------------------------
...
def launch_worker_process(job_file_path):
    ...
    try:
        result = subprocess.run(
            ...
            text=True, env=clean_env, timeout=3600
        )
...
if __name__ == '__main__':
    ...
    if '--worker' in sys.argv:
        ...
        print(
            f"--> [Worker PID: {os.getpid()}] Processing Frame {job_data['frame_idx']:03d} | "
            f"Source: {os.path.basename(job_data['file_path'])}", flush=True
        )

        try:
            renderer = ParaViewRenderer(job_data['config'])
            renderer.render_file(
                job_data['file_path'], job_data['output_filename'], job_data['frame_idx'],
    ...
    # ==========================================================================
        ...
        available_files = [
            os.path.join(r, f)
            for r, _, fs in os.walk(config['inp_dir'])
            for f in fs if f.endswith(('.pvti', '.vti'))
        ]
