    ...
    for h5_path in h5_filepaths:
        ...
        for k in range(4):
            ...
            if k!=0:
                ...
                # ====================================================
                # TEST 1: COUNTER-CLOCKWISE (CCW) 
                # ====================================================
                
                # Spatial rotation CCW (X towards Y)
                exp_edt = torch.rot90(edt_0, k=k, dims=(2, 1))
                exp_uz  = torch.rot90(uz_0,  k=k, dims=(2, 1))
                exp_pr  = torch.rot90(pr_0,  k=k, dims=(2, 1))
    
                # Vector rotation CCW
                if k == 1:   # 90 degrees CCW
                    exp_ux = torch.rot90(-uy_0, k=k, dims=(2, 1))
                    exp_uy = torch.rot90(ux_0,  k=k, dims=(2, 1))
                elif k == 2: # 180 degrees CCW
                    exp_ux = torch.rot90(-ux_0, k=k, dims=(2, 1))
                    exp_uy = torch.rot90(-uy_0, k=k, dims=(2, 1))
                elif k == 3: # 270 degrees CCW
                    exp_ux = torch.rot90(uy_0,  k=k, dims=(2, 1))
                    exp_uy = torch.rot90(-ux_0, k=k, dims=(2, 1))
    
                # Calculate Max Absolute Errors for CCW
                ...
                test_ccw_fails = (sum([err_edt, err_uz, err_ux, err_uy, err_pr]) > 1e-6)
                ...
                # Spatial rotation CW (Y towards X)
                exp_edt = torch.rot90(edt_0, k=-k, dims=(2, 1))
                exp_uz  = torch.rot90(uz_0,  k=-k, dims=(2, 1))
                exp_pr  = torch.rot90(pr_0,  k=-k, dims=(2, 1))
                
                # Vector rotation CW
                if k == 1:   # 90 degrees CW
                    exp_ux = torch.rot90(uy_0,  k=-k, dims=(2, 1))
                    exp_uy = torch.rot90(-ux_0, k=-k, dims=(2, 1))
                elif k == 2: # 180 degrees CW
                    exp_ux = torch.rot90(-ux_0, k=-k, dims=(2, 1))
                    exp_uy = torch.rot90(-uy_0, k=-k, dims=(2, 1))
                elif k == 3: # 270 degrees CW
                    exp_ux = torch.rot90(-uy_0, k=-k, dims=(2, 1))
                    exp_uy = torch.rot90(ux_0,  k=-k, dims=(2, 1))
    
                # Calculate Max Absolute Errors for CW
                err_edt = (edt_k - exp_edt).abs().max().item()
                err_uz  = (uz_k - exp_uz).abs().max().item()
                err_ux  = (ux_k - exp_ux).abs().max().item()
                err_uy  = (uy_k - exp_uy).abs().max().item()
                ...
                test_cw_fails = (sum([err_edt, err_uz, err_ux, err_uy, err_pr]) > 1e-6)
                
                
                # ====================================================
                # FINAL VERIFICATION
                # ====================================================
                if test_ccw_fails and test_cw_fails:
                    raise Exception(f"  Augmentation applied incorrectly on sample {k*90}°.")
                else: 
                    # Optional: you can print WHICH one it passed!
                    if not test_ccw_fails:
                        print(f"  Sample {k*90}° passed (CCW geometry).")
                    else:
                        print(f"  Sample {k*90}° passed (CW geometry).")
...
datasets_to_test = [
    ...
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Silveira_SphPore_SAug_DNorm.h5", 
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Silveira_SphGrain_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_Bentheimer_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_Leopard_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_CastleGate_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_BereaUpperGray_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_BereaSinterGray_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_BereaBuff_SAug_DNorm.h5",
    "../NN_Datasets_Grad_Dist_40_5_55/Train_Oliveira_Berea_SAug_DNorm.h5",
